...
function DrawingSlideOver({
  ...
  async function handleRegionExtract(bbox: BoundingBox, fieldName: string, imageBase64?: string | null) {
    try {
      const res = await fetch(`/api/drawings/${drawing.id}/cross-check-region`, {
        method: "POST",
      ...
      if (data.extractedValue) {
        // Save to current drawing immediately
        onSave(drawing.id, fieldName, data.extractedValue);
        ...
        if (manualVal) {
          onSave(drawing.id, fieldName, manualVal);
          setCrossCheckResult({
            bbox,
            fieldName,
  ...
  async function handleBulkApply(applyToAll: boolean) {
    ...
    try {
      ...
      onReload(); // Refresh the table behind the modal
    } catch {
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="relative ml-auto w-[85vw] max-w-none flex bg-white shadow-2xl animate-slide-in">
        ...
        <div className="flex-1 bg-gray-800 min-w-0">
          <PDFViewer
            ...
            onRegionExtract={handleRegionExtract}
          />
        </div>
      ...
      {crossCheckResult && (
        <div className="absolute inset-0 bg-gray-900/40 z-[60] flex items-center justify-center p-4">
